...
describe('KlarnaCreateOrderService', () => {
  let fixture: KlarnaCreateOrderServiceFixture;

  beforeAll(() => {
    fixture = new KlarnaCreateOrderServiceFixture();
  });
...
class KlarnaCreateOrderServiceFixture {
  createOrderService = null;
  ...
  async whenCreateRequestIsCalled(): Promise<void> {
    ...
    const sessionRequest = SessionRequestBuilder.buildSessionRequest(sessionRequestJSON);
    const serverUrl = 'http:localhost:8082';
    const authorization_token = '36a1b431-db01-2ada-9a0f-f53c11851830';
    this.jsonResponse = await this.createOrderService.sendRequest(sessionRequest, authorization_token, serverUrl);
  }

  thenCreateSessionRequestIsSent(): void {
    expect(this.remoteSend.sendPostRequest).toHaveBeenCalledWith('http:localhost:8082/payments/v1/authorizations/36a1b431-db01-2ada-9a0f-f53c11851830/order/',
      '{"purchase_country":"SE","purchase_currency":"SEK",' +
      '"locale":"en-SE","merchant_reference1":"merch_ref1_123",' +
      '"merchant_reference2":"merch_ref2_123","order_amount":10000,"order_tax_amount":2000,"order_lines":[' +
      '{"type":"physical","reference":"123050","name":"Tomatoes","quantity":10,"quantity_unit":"kg","unit_price":1000,' +
      '"tax_rate":2000,"total_amount":10000,"total_tax_amount":1666}]}');
