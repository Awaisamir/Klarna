...
export class MockKlarnaCancelOrderService {
  constructor(private windowRef: WindowRef) {
  }

  async sendRequest(orderId: string): Promise<any> {

    if (this.windowRef.phoneNumber === '07929633567') {
      throw new ServerError(403, 'Cancel not allowed');
    }
    return new Promise<Response>((resolve): void => {
      resolve(this.createCancelResponse());
    });
  }

  createCancelResponse(): any {
    const obj = {
