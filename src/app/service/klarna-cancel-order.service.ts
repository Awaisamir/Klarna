...
import {MockKlarnaCancelOrderService} from './mock-klarna-cancel-order.service';
import {WindowRef} from './window.service';
import {CaptureOrderRequest} from '../datamodel/klarna-capture-order.model';

@Injectable({providedIn: 'root'})
export class KlarnaCancelOrderService {

  constructor(private remoteSend: KlarnaRemoteSend, private settingService: SettingsService,
              private mockKlarnaCancelOrderService: MockKlarnaCancelOrderService, private windowRef: WindowRef) {
  }

  async sendRequest(orderId: string, serverUrl: string): Promise<any> {
    if (this.settingService.isMockDataMode()) {
      return this.mockKlarnaCancelOrderService.sendRequest(orderId);
    }

    const response = await this.remoteSend.sendPostRequest(`${serverUrl}/ordermanagement/v1/orders/${orderId}/cancel/`, '');
