...
export class MockHppSessionService {
  constructor(private windowRef: WindowRef) {
  }
  async sendRequest(inputJson: string, serverUrl: string): Promise<any> {
  ...
  createResponse(): any {
    if (this.windowRef.phoneNumber === '07929633666') {
      throw new ServerError(400, 'Server Error');
    }
    else if (this.windowRef.phoneNumber === '07929633777') {
